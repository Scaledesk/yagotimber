...
/**
	...
	public function exportProductData($product, $export_multi_store, $websiteCode="")
    {
        ...
        foreach ($entityIds as $i => $dataentityId) {
			...
			if ($overallcount < $recordlimitend && $overallcount >= $recordlimitstart) {
				...
				if($this->getVar('export_multi_store') == "true") {	
					...
					if ($this->getStore()->getCode() == Mage_Core_Model_Store::ADMIN_CODE) {
						...
						foreach ($allproduct->getWebsiteIds() as $websiteId) {
							...
							foreach ($allstoreids as $storeId) {
								...
								$this->exportProductData($product, $this->getVar('export_multi_store'), $websiteCode);
							}
							$overallcount+=1;
						}
					...
					$this->setProductTypeInstance($product);
					/* @var $product Mage_Catalog_Model_Product */
					$this->exportProductData($product, $this->getVar('export_multi_store'));	
