...
class Apptha_Marketplace_ProductController extends Mage_Core_Controller_Front_Action {
    ...
    public function newpostAction() {
         ...
         */
        $sku = $skuFirst = $skuSecond = $skuThird = $productNameTrim = $set = $setbase = $type = $store = $sellerId = $comparePrice = '';
        /**
    ...
    public function editpostAction() {
        ...
        $compare = $this->getRequest()->getPost('compare_product');
        if(!empty($compare))
            {
              	$productArray =explode(',',$compare);
              	foreach ($productArray as $product)
            	{
               		$collection=Mage::getModel('catalog/product')->getCollection()->addAttributeToFilter('name',$product)->getFirstItem()->getId();
               		if(!is_null($collection))
               		{
               			$products=Mage::getModel('catalog/product')->loadByAttribute('name',$product)->getId();
        	      		$prod[]=$products;}
        	       	}
        	 		$comparePrice=implode(',',$prod);
              }
