...
class Apptha_Marketplace_Model_Sellerprofile extends Mage_Core_Model_Abstract
{
    ...
    function addRewriteUrl($storeName, $sellerId) {
        ...
        Mage::getModel('core/url_rewrite')
                ->setIsSystem(0)
                ->setIdPath('seller/' . $sellerId)
                ->setTargetPath('marketplace/seller/displayseller/id/' . $sellerId)
   ...
   function sellerProduct($id){   		
    ...
    function getTodayOrder($id){
    	...
    	$collection = Mage::getModel('marketplace/commission')->getCollection()
    	->addFieldToFilter('seller_id',$id)
    	->addFieldToFilter('created_at', array('from' => $currentDate,'to' => $yesterDay));
    	return $collection;
    }
    /**
    ...
    function getYesterdayOrder($id){
    	...
    	$collection = Mage::getModel('marketplace/commission')->getCollection()
    			->addFieldToFilter('seller_id',$id)
    			->addFieldToFilter('created_at', array('from' => $dayBefore,'to' => $yesterDay));    	
    	return $collection;
    }
    /**
    ...
    function getLastWeekIncome($id){
    	...
    	$collection = Mage::getModel('marketplace/commission')->getCollection()
    	->addFieldToFilter('seller_id',$id)
    	->addFieldToFilter('created_at', array('from' => $fromDisplay,'to' => $toDisplay));
    	return $collection;
    }
    /**
