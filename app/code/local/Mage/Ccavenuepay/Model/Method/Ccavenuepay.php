...
class Mage_Ccavenuepay_Model_Method_Ccavenuepay extends Mage_Payment_Model_Method_Abstract
{
	...
	public function getStandardCheckoutFormFields($option = '')
    {
	  ...
	  $order_ashu =	 Mage::getModel('sales/order')->loadByIncrementId($data['Order_Id']);
		

	 $quote_id_get =  $order_ashu->getQuoteId();
 
	 $quote_ashu = Mage::getModel('sales/quote')->load($quote_id_get);
  ...
  $ashu_price_get =  $quote_ashu->getData('subtotal');
   ...
   else
   {
	 $ashu_price= $ashu_price_get;	  
	    ...
	    Mage::log('Start Ashu partial payment here'); 
		 Mage::log($cc_option);
		  Mage::log('Ashu partial payment end here');
